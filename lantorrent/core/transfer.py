...
class TransferProtocol:
    ...
    def __init__(self, peer_manager, file_manager):
        ...
        self._scheduler_task = None  # Background scheduler task
        # Tracks bytes downloaded from each peer for each file: {file_hash: {peer_id: bytes_count}}
        self.bytes_downloaded_from_peers_for_file = {}

    async def start(self):
        """Start the transfer protocol server and scheduler."""
    ...
    async def stop(self):
        """Stop the transfer protocol server and cleanup."""
        self.running = False
    ...
    async def _process_chunk_requests(self):
        ...
        for request in list(self.active_transfers):
            # Remove requests for files no longer being actively downloaded
